...
class BaseNode(TSLIF_base.MemoryModule):
    def __init__(self,
        ...
        self.alpha_s = torch.nn.Parameter(torch.randn([1, 1, 11], dtype=torch.float))
        self.alpha_l = torch.nn.Parameter(torch.randn([1, 1, 11], dtype=torch.float))
    ...
    def v_float_to_tensor(self, x: torch.Tensor):
        if isinstance(self.v, float):
            v_init = self.v
            self.v = torch.full_like(x.data, 0)

        for i in range(1, self.k + 1):
            name = 'v' + str(i)
            if isinstance(self.names[name], float):
                self.names[name] = torch.full_like(x, fill_value=0)
...
class TSLIFNode(BaseNode):
    ...
    def neuronal_charge(self, x: torch.Tensor):
        v1, v2 = self.names['v1'], self.names['v2']

        v1_new = self.decay_factor[0] * v1 + self.decay_factor[1] * x - self.yy * v2
        v2_new = self.decay_factor[2] * v2 + self.decay_factor[3] * x - self.kk * v1_new
    ...
    def neuronal_reset(self, spike_s, spike_l):
        if not self.hard_reset:
            ...
            self.names['v1'] = self.jit_hard_reset(self.names['v1'], spike_l, self.v_reset)
            self.names['v2'] = self.jit_hard_reset(self.names['v2'], spike_s, self.v_reset)
